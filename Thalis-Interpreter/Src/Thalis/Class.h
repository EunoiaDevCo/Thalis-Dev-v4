...
public:
	Class(const std::string& name) :
		m_Name(name), m_NextFunctionID(0) { }
	...
	uint64 CalculateMemberOffset(Program* program, const std::vector<std::string>& members, TypeInfo* typeInfo, uint32 currentMember = 0, uint32 currentOffset = 0);
	uint64 CalculateStaticOffset(Program* program, const std::vector<std::string>& members, TypeInfo* typeInfo);
